...
def activate():
    ...
    if not license_key or not device_id:
        return jsonify({"ok": False, "error": "key e device_id são obrigatórios"}), 400
    ...
    if lic["device_id"] and lic["device_id"] != device_id:
        conn.close()
        return jsonify({"ok": False, "error": "licença já usada em outro dispositivo"}), 403
        ...
        if password != ADMIN_PASSWORD:
            ...
            if action == "create" and plano_valido(plan):
                ...
                cur.execute(
                    """
                    UPDATE licenses
                    SET device_id = NULL, activated_at = NULL, expires_at = NULL, status = 'active'
                    WHERE license_key = ?
                    """,
                    (key,),
                )
